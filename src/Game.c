...
Game Game_init(const char* filepath, Player* player, array* entities, array* items, bool combat, bool collide, bool end)
{
    ...
    {
        Entity* oent = (Entity*)aget(entities, i);
        Entity nent = entity_init(oent->_symbol, oent->_xposition, oent->_yposition, oent->_damage, oent->_id);
        ainsert(&game._current_entities, &nent);
        game._current_entities_amount++;
    }

    for (size_t i = 0; i < alength(items); i++)
    {
        Item* oitm = (Item*)aget(items, i);
        Item nitm = item_init(string_getf(&oitm->_name), oitm->_xposition, oitm->_yposition, oitm->_damage);
        ainsert(&game._current_items, &nitm);
